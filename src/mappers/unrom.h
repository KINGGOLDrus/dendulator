...
#define NES_MAPPER_ID_UNROM 2

static uint8_t nes_mem_read_unrom(nes_t *nes, uint16_t addr) {
...
static void nes_mem_write_unrom(nes_t *nes, uint16_t addr, uint8_t val) {
  ...
  if (addr >= 0x8000) {
    uint8_t bank_id = (nes->cart.rom16_count > 8) ? (val & 0x0F) : (val & 0x07);

    if (bank_id < nes->cart.rom16_count)
      nes->mem.prg[1] = nes->cart.rom[bank_id];
    else
      nes->mem.prg[1] = NULL;
...
static uint8_t nes_vmem_read_unrom(nes_t *nes, uint16_t addr) {
  addr &= 0x3FFF;
  if (addr >= 0x3F00) {
    addr &= 0x001F;
...
static void nes_vmem_write_unrom(nes_t *nes, uint16_t addr, uint8_t val) {
  addr &= 0x3FFF;
  if (addr >= 0x3F00) {
    addr &= 0x001F;
    if (addr >= 0x10 && (addr & 0x03) == 0)
...
static void nes_init_unrom(nes_t *nes) {
...
static void nes_cleanup_unrom(nes_t *nes) {
  return;
}
...
MAPPER_REG_FUNC
static void nes_register_unrom() {
  static const char *mapper_name = "UNROM";
  static nes_mapper_funcs_t mapper_funcs = {
    .init = nes_init_unrom, .cleanup = nes_cleanup_unrom,
    .read = nes_mem_read_unrom, .write = nes_mem_write_unrom,
    .vread = nes_vmem_read_unrom, .vwrite = nes_vmem_write_unrom,
  };

  nes_reg_mapper(NES_MAPPER_ID_UNROM, mapper_name, &mapper_funcs);
}

MAPPER_UNREG_FUNC
static void nes_unregister_unrom() {
  nes_unreg_mapper(NES_MAPPER_ID_UNROM);
}
